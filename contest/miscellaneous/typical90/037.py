...
def main():
    ...
    dp: List[List[int]] = [[-(1 << 60) for j in range(W + 1)] for i in range(N + 1)]
    dp[0][0] = 0

    seg = SegmentTree[int](
        lambda x, y: max(x, y), lambda: -(1 << 60), [-(1 << 60)] * (W + 1)
    )
    seg.set(0, 0)
    ...
    for i in range(1, N + 1):
        ...
        for j in range(W + 1):
            left: int = max(0, j - R[i - 1])
            right: int = max(0, j - L[i - 1] + 1)

            if left == right:
                continue

            prod: int = seg.prod(left, right)
            if prod == -(1 << 60):
                continue

            dp[i][j] = max(dp[i][j], prod + V[i - 1])
