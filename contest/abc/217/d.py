...
def main():
    ...
    X: List[int] = compress([x for c, x in query])

    seg_r = SegmentTree[int](
        lambda x, y: min(x, y), lambda: 1 << 60, [1 << 60] * len(X)
    )
    seg_r.set(len(X) - 1, L)
    seg_l = SegmentTree[int](lambda x, y: max(x, y), lambda: 0, [0] * len(X))
    seg_l.set(0, 0)

    for (c, x), i in zip(query, X):
        if c == 1:
            seg_l.set(i, x)
            seg_r.set(i, x)
        else:
            r = seg_r.prod(i, len(X))
            l = seg_l.prod(0, i)
            print(r - l)
