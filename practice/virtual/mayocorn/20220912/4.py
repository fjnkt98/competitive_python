...
def main():
    ...
    C, X = map(list, zip(*[list(map(int, input().split())) for i in range(Q)]))

    Z: List[int] = [0] + X + [L]
    Y: List[int] = compress(Z)
    x_to_i: Dict[int, int] = {x: y for x, y in zip(Z, Y)}

    seg_max = SegmentTree[int](
        lambda a, b: max(a, b), lambda: -1, [-1 for i in range(len(Y))]
    )
    seg_min = SegmentTree[int](
        lambda a, b: min(a, b), lambda: 1 << 60, [1 << 60 for i in range(len(Y))]
    )
