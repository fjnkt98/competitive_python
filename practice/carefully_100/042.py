...
def main():
    ...
    D: List[int] = [int(input()) for i in range(N)]
    C: List[int] = [int(input()) for i in range(M)]
    ...
    dp: List[List[int]] = [[e() for j in range(M + 1)] for i in range(N)]
    dp[0] = [e()] + [c * D[0] for c in C]

    seg = SegmentTree[int](op, e, dp[0])

    for i in range(1, N):
        for j in range(i + 1, M + 1):
            dp[i][j] = min(dp[i][j], seg.prod(i, j) + D[i] * C[j - 1])

        seg = SegmentTree[int](op, e, dp[i])
